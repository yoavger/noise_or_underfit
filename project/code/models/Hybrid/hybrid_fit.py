...
def hybrid_fit(df,num_of_parameters_to_recover):
    """
    this funcation performs parameters recovery of hybrid agent on the *reduce* two-stage task  
    Args:
...
def parameters_recovary(parameters, df):
    ...
    for t in range(num_of_trials):
        ...
        choices_probs[t] = np.exp(beta*(q_net[action_1])) / np.sum(np.exp(beta*(q_net))) 
